...
# dF/dN|N=dM/b-d => (b-d)2/bM #the numerator will always remain postive, stable when (b-d)^2 does equal 0 (b cannot be equal to d) and when bM is less than 0. Therefore, never stable.
...
# Equilbrium points: N*=0, N*=10, N*=50
...
f <- ggplot(data.frame(x = c(-50, 100)), aes(x)) + xlab("N size") + labs(title = "function: -b*x/(x+M) - d - c*x ")
f + stat_function(fun = test) +geom_point(data=newpoints, aes(x=x, y=y), shape=1)
